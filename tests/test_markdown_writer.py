...
from screener.markdown_writer import (
    _format_decision_with_indicator,
    _normalize_headings,
...
def test_bluf_at_top_with_signal_and_excerpt():
    ...
    # Buy → green check + label, both in BLUF and footer
    assert "✅ `Buy`" in md
...
def test_handles_completely_empty_state():
    ...
    # Hold → yellow square + label
    assert "🟨 `Hold`" in md
    # Footer always present
    assert "Generated by TradingAgents screener pipeline" in md


# --- Rating-indicator helper -----------------------------------------------


def test_format_decision_with_indicator_maps_all_five_ratings():
    """Per user spec: Buy=5/✅, Overweight=4/🟢, Hold=3/🟨,
    Underweight=2/🟧, Sell=1/❌."""
    assert _format_decision_with_indicator("Buy") == "✅ `Buy`"
    assert _format_decision_with_indicator("Overweight") == "🟢 `Overweight`"
    assert _format_decision_with_indicator("Hold") == "🟨 `Hold`"
    assert _format_decision_with_indicator("Underweight") == "🟧 `Underweight`"
    assert _format_decision_with_indicator("Sell") == "❌ `Sell`"


def test_format_decision_with_indicator_is_case_insensitive():
    """Decision can come through in any casing depending on the code path
    (PortfolioRating enum is PascalCase, but signal_processing might
    upper-case for display). Match all variants."""
    assert _format_decision_with_indicator("BUY") == "✅ `BUY`"
    assert _format_decision_with_indicator("hold") == "🟨 `hold`"
    assert _format_decision_with_indicator("OvErWeIgHt") == "🟢 `OvErWeIgHt`"


def test_format_decision_with_indicator_handles_unknown_rating():
    """Novel ratings (or LLM hallucinations) render without an indicator
    rather than getting a wrong one. Backticked label stays."""
    assert _format_decision_with_indicator("Maybe") == "`Maybe`"
    assert _format_decision_with_indicator("STRONG BUY") == "`STRONG BUY`"


def test_format_decision_with_indicator_handles_empty_or_none():
    assert _format_decision_with_indicator("") == "`?`"
    assert _format_decision_with_indicator(None) == "`?`"


def test_render_applies_indicator_in_both_bluf_and_footer():
    """The visual indicator must appear in both the BLUF Signal line and
    the footer Decision line — both are user-facing summaries."""
    md = render_markdown_report(
        ticker="X", trade_date="2026-05-03", state={}, decision="Sell",
    )
    bluf, footer = md.split("---")[0], md.split("---")[-1]
    assert "❌ `Sell`" in bluf
    assert "❌ `Sell`" in footer
